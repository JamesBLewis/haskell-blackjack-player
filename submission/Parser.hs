...
(|||) p1 p2 = P $ \i -> case parse p1 i of
  Result rest1 x1 -> Result rest1 x1
  Error e1 -> parse p2 i
...
satisfy f = do
  c <- character
  let bool = if f c then pure c else unexpectedCharParser c
  bool
...
--
-- /Hint/: Use the monad instance and you must change the answer to use do notation.
--
-- >>> parse (tok (is 'a')) "a bc"
-- Result >bc< 'a'
--
